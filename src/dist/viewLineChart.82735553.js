...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"scripts/viewLineChart.js":[function(require,module,exports) {
fetchDataCases();

function filterData(data) {
  var maxTempYear = data.filter(function (item) {
    return item.name === 'Jahreshöchsttemperatur';
  });
...
function visualiseChart(data) {
  ...
  var xAxis = d3.scaleLinear().domain(d3.extent(data, function (item) {
    return Number(item.jahr);
  })).range([0, width]);
  svg.append("g").attr("transform", "translate(0, ".concat(height, ")")).call(d3.axisBottom(xAxis));
  var yAxis = d3.scaleLinear().domain([d3.min(data, function (item) {
    return Number(item.wert);
  }) - 1, d3.max(data, function (item) {
    return Number(item.wert);
  })]).range([height, 0]);
  ...
  var curve = svg.append("path").datum(data).attr("fill", "none").attr("stroke", "turquoise").attr("stroke-width", 3).attr("d", d3.line().x(function (item) {
    return xAxis(Number(item.jahr));
  }).y(function (item) {
    return yAxis(Number(item.wert));
  }));
  d3.select("#mySlider").on("change", function (d) {
    selectedValue = this.value;
    xAxis.domain([selectedValue, d3.max(data, function (item) {
      return Number(item.jahr);
    })]).range([0, width]);
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","scripts/viewLineChart.js"], null)
//# sourceMappingURL=/viewLineChart.82735553.js.map